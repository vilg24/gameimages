...
let selectedPlayerCharacterId = "galaxybro";
let selectedEnemyCharacterId = "lightningkid";
let pvpPickTurn = "player";
let crowdVoice = null;
...
function chooseCharacter(characterId) {
  ...
  if (gameMode === "cpu") {
    ...
    selectedEnemyCharacterId = pickOpponentCharacterId(characterId);
    awaitingCpuChoice = false;
    syncModeUi();
    if (setupSummaryEl) {
      const charName = CHARACTER_LIBRARY[characterId].name;
      setupSummaryEl.textContent = `Vs Computer selected. You chose ${charName}.`;
    }
    showSetupPage(3);
    return;
  }

  if (pvpPickTurn === "player") {
    selectedPlayerCharacterId = characterId;
    pvpPickTurn = "enemy";
    awaitingCpuChoice = true;
    syncCharacterSelectionUi();
  ...
  selectedEnemyCharacterId = characterId;
  pvpPickTurn = "player";
  awaitingCpuChoice = false;
  syncModeUi();
  if (setupSummaryEl) {
    const p1 = CHARACTER_LIBRARY[selectedPlayerCharacterId].name;
    const p2 = CHARACTER_LIBRARY[selectedEnemyCharacterId].name;
    setupSummaryEl.textContent = `2 Players selected. Player 1: ${p1}. Player 2: ${p2}.`;
  }
  showSetupPage(3);
}
...
function initSetupFlow() {
  ...
  awaitingCpuChoice = true;
  pvpPickTurn = "player";
  selectedPlayerCharacterId = "galaxybro";
  selectedEnemyCharacterId = "lightningkid";
...
function syncCharacterSelectionUi() {
  ...
  if (gameMode === "cpu") {
    setupCharacterTitleEl.textContent = "2. Choose Your Character";
    setupCharacterInstructionEl.textContent = "Choose your fighter (computer will get a different character).";
    return;
  }
  setupCharacterTitleEl.textContent = "2. Choose Characters";
  if (pvpPickTurn === "player") {
    setupCharacterInstructionEl.textContent = "Player 1: choose your character.";
  } else {
    setupCharacterInstructionEl.textContent = "Player 2: choose your character.";
  }
}
...
function syncModeUi() {
  if (gameMode === "cpu") {
    ...
    enemyPokemon.pilot = "Computer";
  } else {
    playerPokemon.pilot = "Player 1";
    enemyPokemon.pilot = "Player 2";
  }
...
function resetBattle() {
  ...
  playerPokemon.blockGuess = null;
  playerPokemon.blockLocked = false;
  playerPokemon.shieldTurns = 0;
  playerPokemon.hackCountdown = 0;
  enemyPokemon.blockGuess = null;
  enemyPokemon.blockLocked = false;
  enemyPokemon.shieldTurns = 0;
  enemyPokemon.hackCountdown = 0;
  locked = false;
  finished = false;
  activeSide = "player";
...
function pickOpponentCharacterId(playerId) {
  const order = ["lightningkid", "galaxybro", "superdad", "starsha", "soccerholden", "eliashacker"];
  return order.find((id) => id !== playerId) || "lightningkid";
}
...
function assignBattleCharacters() {
  const playerId = CHARACTER_LIBRARY[selectedPlayerCharacterId] ? selectedPlayerCharacterId : "galaxybro";
  const enemyId = CHARACTER_LIBRARY[selectedEnemyCharacterId] ? selectedEnemyCharacterId : pickOpponentCharacterId(playerId);
  applyCharacter(playerPokemon, playerId);
  applyCharacter(enemyPokemon, enemyId);
}
...
function handleTurn(attackIndex) {
  ...
  if (applyHackTick(attacker)) {
    syncUi();
    endBattle(`${defender.pilot} wins with ${defender.name}!`);
    return;
  }
...
function handleSuperPower(attacker) {
  ...
  if (attacker.specialType === "galaxy_combo") {
    ...
    if (defender.shieldTurns > 0) {
      ...
      result = {
        text: `${attacker.name} SPECIAL healed 50 HP and hit for 50 damage (${attacker.hp}/${MAX_HP} HP, ${defender.name} ${defender.hp}/${MAX_HP} HP).`,
        kind: "damage",
        hits: 1
      };
    }
    ...
    hits += bonusHits;
    const damage = hits * perHitDamage;
    ...
    if (defender.shieldTurns > 0) {
      ...
      defender.hp = Math.max(0, defender.hp - damage);
      result = {
        text: `${text} (${defender.name} at ${defender.hp}/${MAX_HP} HP).`,
        kind: "damage",
        hits
      };
    }
    ...
    if (defender.shieldTurns > 0) {
      ...
      defender.hp = Math.max(0, defender.hp - damage);
      result = {
        text: `Elias hacked the game for ${damage} damage (${defender.name} at ${defender.hp}/${MAX_HP} HP).`,
        kind: "damage",
        hits: 1
      };
    ...
    const damage = Math.ceil(MAX_HP * 0.75);
    if (defender.shieldTurns > 0) {
      defender.shieldTurns -= 1;
      result = {
      ...
      result = { text: `SUPER POWER hit for ${damage} damage (${defender.name} at ${defender.hp}/${MAX_HP} HP).`, kind: "damage", hits: 1 };
    }
...
function performAttack(attacker, defender, attack) {
  ...
  if (crit) {
    return {
      text: `Critical hit for ${damage} damage${comboText} (${defender.name} at ${defender.hp}/${MAX_HP} HP).`,
      kind: "damage",
      hits
    };
  }
  return {
    text: `${damage} damage dealt${comboText} (${defender.name} at ${defender.hp}/${MAX_HP} HP).`,
    kind: "damage",
    hits
  };
}
